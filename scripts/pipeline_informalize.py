...
async def process_problems(
    ...
    for i, item in enumerate(tqdm(ds)):
        ...
        if not item.get("formal_statement"):
            logger.warning(f"Skipping item {i} due to missing 'formal_statement'")
            continue

        code = remove_comments(item["formal_statement"])

        messages = [
            {"role": "system", "content": read_system_prompt("informalize")},
...
async def main():
    """Main async function."""
    # Load configuration
    config = read_config("configs/informalization.yaml")
    ...
    logger.info("Starting Informalizing Process")
