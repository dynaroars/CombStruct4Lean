...
def process_lean_file(file_contents, byte_idx_1, byte_idx_2):
    """
    Process a Lean file to extract content between two byte indices and convert positions.
    
    Args:
        file_contents (str): The contents of the Lean file
        byte_idx_1 (int): Starting byte index
        byte_idx_2 (int): Ending byte index
    
    Returns:
        tuple: (extracted_string, line_1, column_1, line_2, column_2, char_idx_1, char_idx_2)
            - extracted_string: The text between the byte indices
            - line_1, column_1: Line and column numbers for start position
            - line_2, column_2: Line and column numbers for end position 
            - char_idx_1, char_idx_2: Character indices for start and end positions
    """
...
def extract_positions(node):
    """
    Extract position information from AST nodes.
    
    Args:
        node (dict or list): AST node or list of nodes
        
    Returns:
        list: List of tuples containing start and end positions
    """
    positions = []
...
def extract_vals(data):
    """
    Extract 'val' fields from AST nodes.
    
    Args:
        data (dict or list): AST node or list of nodes
        
    Returns:
        list: List of stripped values from 'val' fields
    """
    vals = []
...
def find_doc_comment_vals(data):
    """
    Find and extract documentation comments from AST nodes.
    ...
    Returns:
        tuple: (vals, positions)
            - vals: List of documentation comment values
            - positions: List of position tuples for each comment
    """
    vals = []
    positions = []
    ...
    if isinstance(data, dict):
        ...
        for value in data.values():
            v, p = find_doc_comment_vals(value)
            vals.extend(v)
            positions.extend(p)
    elif isinstance(data, list):
        for item in data:
            v, p = find_doc_comment_vals(item)
            vals.extend(v)
            positions.extend(p)
...
def find_attributes_vals(data):
    """
    Find and extract attribute values from AST nodes.
    ...
    Returns:
        tuple: (vals, positions)
            - vals: List of attribute values
            - positions: List of position tuples for each attribute
    """
    vals = []
    positions = []
...
def find_pripro_vals(data):
    """
    Find and extract private/protected modifiers from AST nodes.
    ...
    Returns:
        tuple: (vals, positions)
            - vals: List of private/protected values
            - positions: List of position tuples for each modifier
    """
    vals = []
    positions = []
...
def extract_other_vals(data):
    """
    Extract all other 'val' fields from AST nodes that don't fall into specific categories.
...
def find_kind_name_theorem_lemma_abbrev_def_instance_inductive(file_content, data):
    """
    Find and extract kind and name information for theorems, lemmas, abbreviations, 
    definitions, instances, and inductive types.
    
    Args:
        file_content (str): The content of the Lean file
        data (dict): AST node
        
    Returns:
        tuple: Contains kind and name information with their positions:
            (kind, kind_line_1, kind_column_1, kind_line_2, kind_column_2, 
             kind_char_idx_1, kind_char_idx_2, name, name_line_1, name_column_1, 
             name_line_2, name_column_2, name_char_idx_1, name_char_idx_2)
    """
    kind = None
    name = None
    kind_pos = None
...
def process_modifier(file_content, declaration, tactics):
    doccomment_vals, doccomment_positions = find_doc_comment_vals(declaration)
    if doccomment_vals:
